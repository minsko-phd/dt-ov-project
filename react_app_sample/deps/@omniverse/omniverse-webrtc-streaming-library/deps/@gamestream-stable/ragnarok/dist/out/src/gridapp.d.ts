import { SessionParams, AuthInfo, PlatformDetails, IEventEmitter, AdUpdate, LegacyLogEmitter } from "./dependencies";
import { EventDataElements, InitParams, StartSessionResultEvent, InputConfigFlags, StreamingTerminatedEvent } from "./interfaces";
...
import { CustomMessage, InputType } from "./nskinterfaces";
/**
 ...
 * call startSession only after successful initialization of grid app using initialize method
...
declare interface IGridApp extends IEventEmitter {
     ...
     *  The Ads information are provided in the SessionUpdate event during session setup process.
     ...
     *      FINISHED -> None
     *      CANCELLED -> None
     *  This API is valid only if a session setup is in progress (startSession API). After all the Ad play is completed and server is ready for streaming the SESSION_START_RESULT event will be sent.
     *  This API can throttle the ads update request to server if there are too many pause/resume updates for an Ad.
     *  Returns RErrorCode.Success if the ad update is successfully accepted.
     *  @param  adUpdate: AdUpdate to be sent to the server. Refer to AdUpdate documentation for the various parameters to be passed.
     *     Note: Ad play can be canceled on the client device for multiple reasons,
     *           In few cases the session might be terminated on server and in few cases session will transition to ready for streaming only after waiting for minimum duration.
     *           Refer to the design document for the cancelReason to be set in AdUpdate
     */
    updateAdState(adUpdate: AdUpdate): RErrorCode;
    /**
     * getSession request.
     ...
     * 4. Client calls getActiveSessions() and if the appid is same and error is (RErrorCode.SessionLimitExceeded || RErrorCode.SessionLimitPerDeviceReached)
     * 5. Client Calls multiple getSession(sessionid) until status becomes SessionState.READY_FOR_CONNECTION
     ...
     * 2. Session is streaming.
     * 3. Client gets STREAM_STOPPED event with error group (((eventData.error.code ^ RErrorCode.StreamerErrorCategory)>>8) == 0)
     * 4. Client Calls multiple getSession(sessionid) until status becomes SessionState.READY_FOR_CONNECTION
     ...
     * When inputs are enabled/disabled, event listeners are registered/de-registered with certain DOM elements.
     * This could be useful when showing overlay button under fullscreen mode.
    ...
    /**
     * Get the reference of VirtualGamepadHandler. Reference can be undefined if input channel failed to go in open state.
...
export declare class GridApp extends LegacyLogEmitter implements IGridApp, IStreamCallbacks {
    ...
    private updateTotalHiddenTime;
    private sendVisibilityMetricsAtExit;
